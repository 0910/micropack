<?php
    ...
    $arrProducts = array();
    if (empty($_GET['cat'])){
        $query = "SELECT idProduct, title2, images, cover, category, orderby FROM products ORDER BY idProduct DESC";
    }
    if (!empty($_GET['cat'])){
        $query = "SELECT idProduct, title2, images, cover, category, orderby FROM products WHERE category = '{$_GET['cat']}' ORDER BY idProduct DESC";
    }
    ...
    foreach ($arrProducts as $Products) {
        ...
        $title = stripslashes($Products['title2']);
        $category = $Products['category'];
        $images = $Products['images'];
        $cover = $Products['cover'];
        $imagesdecoded = json_decode($images);
        $bufferProducts .= '<div class="col-md-3 col-sm-6 item product spacer lazy '. $category .'">
                                <div class="row">
                                    <div class="col-md-12">
                ...
                <div class="row filters">
                    <div class="row">
                        <div class="col-md-3 filter" data-filter=".mega">
                            <div class="filterbox">
                                <a href="experiencias.php?cat=mega">Mega Displays</a>
                            </div>
                        </div>
                        <div class="col-md-3 filter" data-filter=".botaderos">
                            <div class="filterbox">
                                <a href="experiencias.php?cat=botaderos">Bins</a>
                            </div>
                        </div>
                        <div class="col-md-3 filter" data-filter=".depie">
                            <div class="filterbox">
                                <a href="experiencias.php?cat=depie">Standing Displays</a>
                            </div>
                        </div>
                        ...
                        <div class="col-md-3 filter" data-filter=".islas">
                            <div class="filterbox">
                                <a href="experiencias.php?cat=islas">Isles</a>
                            </div>
                        </div>
                        <div class="col-md-3 filter" data-filter=".mostrador">
                            <div class="filterbox">
                                <a href="experiencias.php?cat=mostrador">Counter Displays</a>
                            </div>
                        </div>
                        <div class="col-md-3 filter" data-filter=".automaticos">
                            <div class="filterbox">
                                <a href="experiencias.php?cat=automaticos">Automatic Displays</a>
                            </div>
                        </div>
