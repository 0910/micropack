<?php
	...
	if (!empty($_GET['idNews'])) {
		$query = "SELECT idNews, date, title2, body2, link, images, cover FROM news WHERE idNews = {$_GET['idNews']}";
		$resultado = mysql_query($query, $dbConn) or die(mysql_error());
		$row = mysql_fetch_assoc($resultado);
		$idNews = $row['idNews'];
		$title = stripslashes($row['title2']);
		$body = stripslashes(nl2br($row['body2']));
		$images = $row['images'];
	    $cover = $row['cover'];
